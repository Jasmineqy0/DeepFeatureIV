...
def create_neural_network(num_layer, input_size, hidden_dim, batch_normalization):
    layers = []
...
def build_net_for_fully_random_iv(simulation_info: Dict[str, Any], model_configs: Dict[str, Any]):
    ...
    treatment_net = create_neural_network(model_configs['layer'], dim_ts, model_configs['hidden_dim'], model_configs['batch_normalization'])
    instrumental_net = create_neural_network(model_configs['layer'], dim_iv, model_configs['hidden_dim'], model_configs['batch_normalization'])
    if dim_cf:
        covariate_net = create_neural_network(model_configs['layer'], dim_cf, model_configs['hidden_dim'], model_configs['batch_normalization'])
    else:
        covariate_net = None
